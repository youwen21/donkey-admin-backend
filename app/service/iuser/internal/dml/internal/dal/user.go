...
func (d *userDal) GetSessionByModel(m *model.User) *gorm.DB {
	...
	if m.Email != "" {
		session.Where("`email` = ?", m.Email)
	}

	if m.Phone != "" {
		session.Where("`phone`= ?", m.Phone)
	}
...
func (d *userDal) GetSessionByForm(f *user_def.UserQueryForm) *gorm.DB {
	...
	if f.SearchKey != "" {
		session.Where("`name` like ? OR `real_name` like ? OR `email` = ? OR `phone` = ?",
			"%"+f.SearchKey+"%", "%"+f.SearchKey+"%", f.SearchKey, f.SearchKey)
	}
...
func (d *userDal) Exec(sql string, values ...interface{}) error {
	session := d.newSession()
